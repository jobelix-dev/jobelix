-- Manual fix for credit security
-- This should be run directly on the remote database

-- Drop old function and constraint if they exist
DROP FUNCTION IF EXISTS public.add_purchased_credits(uuid, integer, text);
DROP FUNCTION IF EXISTS public.add_purchased_credits(uuid, integer, text, text, text, integer, text);
DROP INDEX IF EXISTS public.idx_purchases_event_id;

-- Add UNIQUE constraint to stripe_event_id
...
COMMENT ON FUNCTION public.add_purchased_credits(UUID, INTEGER, TEXT, TEXT, TEXT, INTEGER, TEXT) 
IS 'Transaction-safe credit addition with idempotency protection. Prevents race conditions and duplicate credits.';
